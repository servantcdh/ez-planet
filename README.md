...
|---------|-------------|-------------|
| [`@servantcdh/labeling-canvas`](#labeling-canvas) | Fabric.js v6 기반 이미지 라벨링 워크스페이스 | JS 22.6KB, CSS 3.0KB |
| [`@servantcdh/cosmos-canvas`](#cosmos-canvas) | ReactFlow + Three.js 코스모스 시각화 | JS 8.0KB, CSS 2.0KB |
...
```bash
npm install @servantcdh/labeling-canvas
```
...
```tsx
import { LabelingWorkspace } from '@servantcdh/labeling-canvas'
...
import {
  ...
  LabelingNavigation,
  LabelingInfoPanel,
} from '@servantcdh/labeling-canvas'
...
```bash
npm install @servantcdh/cosmos-canvas
```
...
```tsx
import { CosmosCanvas } from '@servantcdh/cosmos-canvas'
import type { PlanetConfig, OrbitConfig, TutorialStep } from '@servantcdh/cosmos-canvas'
...
ez-planet-canvas/
├── packages/
│   ├── labeling-canvas/          # @servantcdh/labeling-canvas
│   │   ├── src/
...
│   │   └── vite.config.ts
│   └── cosmos-canvas/            # @servantcdh/cosmos-canvas
│       ├── src/
